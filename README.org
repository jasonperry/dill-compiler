...
- The explicit mutation tracking should mean that we can avoid
  ...
  compiler to determine that behavior will be correct and use values or
  references as appropriate for efficiency.
...
- Keyword-based block syntax instead of curly braces. Some pains have
  ...
  necessary." Making code easy to type and even pronounce are also goals.
- Eventually: generics. I originally thought of doing interface
  constraints, but now am thinking of having no interfaces at all and
  letting closure passing be used for generic operators.
