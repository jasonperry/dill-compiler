...
let handle_parse_errors (buf: Sedlexing.lexbuf) = function
  | Lexer.Error msg ->
     let open Sedlexing in
     let spos, epos = lexing_positions buf in
...
let parse_sourcefile channel filename =
  ...
  let lexer = Sedlexing.with_tokenizer Slexer.token lexbuf in
  Sedlexing.set_filename lexbuf filename;
  (* Have to create a parsing function to use Sedlex, because the built-in one
     that's generated by Menhir only takes a Lexing.lexbuf. *)
  let srcParser =
    MenhirLib.Convert.Simplified.traditional2revised Parser.dill_source in
  (* let buf = Lexing.from_channel channel in *)   
  try
    (* Parser.dill_source Slexer.token buf *)
    srcParser lexer
  with
  | exn -> handle_parse_errors lexbuf exn
...
let parse_modspec channel filename =
  ...
  let lexer = Sedlexing.with_tokenizer Slexer.token lexbuf in
  let specParser = 
    MenhirLib.Convert.Simplified.traditional2revised Parser.modspec in
  try
    specParser lexer
  with
  | exn -> handle_parse_errors lexbuf exn
